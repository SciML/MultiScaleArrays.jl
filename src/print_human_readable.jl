...
function toprint_AbstractMultiScaleArray!(
        toprint, X::AbstractMultiScaleArray, fields,
    ...
    return if level < levelmax
        for x in X.nodes
            if fieldname(typeof(x), 1) == :nodes
                ...
                toprint_AbstractMultiScaleArray!(
                    toprint, x, fields, levelmax,
...
```
"""
function print_human_readable(
        X::AbstractMultiScaleArray; n_char_per_name = 6,
        fields = nothing, levelmax = Inf, n_item_max_per_levels = Inf
    ) # fields = nothing OR [field1,field2,...]
    #     if X isa AbstractMultiScaleArrayLeaf
    #         println(X)
    #     else
