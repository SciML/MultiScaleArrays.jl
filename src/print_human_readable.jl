...
function toprint_AbstractMultiScaleArray!(toprint, X::AbstractMultiScaleArray, fields, levelmax,
                    ...
                    if !(typeof(x) <: AbstractMultiScaleArrayLeaf)
                        ...
                        push!(toprint[level],
                              join([take_first_char(string(field), n_char_per_name) * ": " *
                                    string(getfield(x, Symbol(field))) for field in collect(fields)],
                                   ", "))
                        first_element = add_separator!(first_element, toprint, level)
                    end
                end
...
```
"""
function print_human_readable(X::AbstractMultiScaleArray; n_char_per_name = 6, fields = nothing,
                              levelmax = Inf, n_item_max_per_levels = Inf) # fields = nothing OR [field1,field2,...]
    #     if typeof(X) <: AbstractMultiScaleArrayLeaf
    #         println(X)
    #     else
    ...
    for level in length(toprint):-1:1
        while occursin("+ ", toprint[level])
            ...
            toprint[level] = join([toprint[level][1:(Move - 1)], toprint[level][(Move + 1):To],
                                   toprint[level][Move], toprint[level][(To + 1):end]])
            for level_ in 1:(level - 1)
                toprint[level_] = join([toprint[level_][1:(Move - 1)],
