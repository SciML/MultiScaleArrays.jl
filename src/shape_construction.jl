...
@generated function similar(m::AbstractMultiScaleArray, ::Type{T} = eltype(m)) where {T}
    ...
    return :(construct(parameterless_type(m), recursive_similar(m.nodes, T), similar(m.values, T),
                       $(assignments...)))
end
...
vcat(m1::AbstractMultiScaleArray, m2::AbstractMultiScaleArray) = error("AbstractMultiScaleArrays cannot be concatenated")

hcat(m1::AbstractMultiScaleArray, m2::AbstractMultiScaleArray) = error("AbstractMultiScaleArrays cannot be concatenated")
